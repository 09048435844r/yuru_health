...
3. [SwitchBot 連携（室内環境）](#switchbot-連携室内環境)
...
- `src.main --auto` で SwitchBot API から `raw_data_lake` に保存
- `src.main --parse-only` で `raw_data_lake` を `environmental_logs` に再構築
- 足跡 UI（最下段 SwitchBot）は **CO2 濃度(ppm)** のスパークラインを表示

補足:

- parse-only では `fetched_at` を優先して timestamp 化するため、同日内の複数点が潰れにくくなっています
- Worker の通常 fetch 周期は `FETCH_INTERVAL_SECONDS`（デフォルト 900秒 = 15分）です
...
#### 足跡の SwitchBot が点だけでグラフにならない

原因候補:

- 当日の CO2 データ点が 1 点以下
- `environmental_logs` への反映不足（raw_data_lake にはあるが parse が追いついていない）

確認コマンド:

```bash
docker compose exec -T worker python -c "from src.database_manager import DatabaseManager; db=DatabaseManager(); rows=db.supabase.table('environmental_logs').select('timestamp,raw_data').eq('source','switchbot').order('timestamp',desc=True).limit(20).execute().data or []; print('rows=',len(rows)); print('co2_points=',sum(1 for r in rows if isinstance(r.get('raw_data'),dict) and r.get('raw_data',{}).get('CO2') is not None))"
```

再構築:

```bash
docker compose exec worker python -m src.main --parse-only --days 7
```

#### 15分ごとに取得されていないように見える

```bash
docker compose exec worker printenv FETCH_INTERVAL_SECONDS
docker compose logs worker --since=30m | grep -E "start fetch|done rc|SwitchBot: environment data saved"
```

- `FETCH_INTERVAL_SECONDS=900` なら 15分周期が期待値です
- 再起動直後は過渡的に短い間隔の1回が混じることがあります
